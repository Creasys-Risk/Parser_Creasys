...
def unify_spaces(s: str) -> str:
    return re.sub(r"\s+", " ", s).strip()

def get_fecha_global(text: str) -> str:
    m = re.search(r"al\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})", text, re.IGNORECASE)
    return m.group(1).strip() if m else ""

def convert_fecha(fecha_str: str) -> datetime.date:
    meses = {
...
def get_metadata_block(text: str):
...
def detect_currency_dynamic(line: str, default_moneda: str) -> str:
    ...
    if "UF" in up_line and "$" in up_line:
        return "UF" if re.search(r"\d+,\d+%\s*", up_line) else "CLP"
    if "UF" in up_line:
        return "UF"
    if "$" in up_line:
        return "CLP"
...
def parse_fondos_inversion_local_clp(line: str):
    p = re.compile(
        r"^(?P<nemotecnico>.+?)\s+"
...
def parse_fondos_mutuos_locales_clp(line: str):
    fm_idx = line.find("FM ")
    if fm_idx == -1:
        return None
...
def parse_fmius_line_regex(line: str):
    pattern = re.compile(
        r"^(?P<nemotecnico>.+?)\s+"
        r"(?P<p1>[\d,]+%)\s+"
...
def parse_fondos_mutuos_internacionales_usd_block(lines: list, start_idx: int):
    data = []
    i = start_idx + 1
    while i < len(lines):
        line = lines[i].strip()
...
def parse_renta_fija_internacional_usd_new(line: str):
    tokens = line.split()
...
def parse_renta_fija_internacional_usd_block(lines: list, start_idx: int):
    data = []
    i = start_idx
    while i < len(lines):
        line = lines[i].strip()
...
def parse_fondos_mutuos_locales_usd(line: str):
    pattern = re.compile(
        r"^(?:[\d,]+\s*%)\s+"
        r"(?P<nemotecnico>FM\s+.+?)(?:\s+(?P<serie>[A-Z]))?\s+"
...
def parse_fondos_mutuos_usd_block(lines: list, start_idx: int):
    data = []
    i = start_idx
    while i < len(lines):
        line = lines[i].strip()
...
def parse_renta_fija_local_clp(line: str):
    p = re.compile(
        r"^(?P<nemo>\S+)\s+"
        r"(?P<emisor>.+?)\s+"
        r"(?P<rating>\S+)\s+"
...
def parse_acciones_local_clp(line: str):
...
def is_new_section_header(line: str) -> bool:
    return any(re.search(pat, line, re.IGNORECASE) for pat in section_patterns)

def parse_section(lines: list, start_idx: int, section_title: str, total_title: str, func: callable, clase: str, default_moneda: str):
    data = []
    i = start_idx
    while i < len(lines):
        line = lines[i].strip()
        ...
        if rec:
            ...
            rec["Moneda"] = detect_currency_dynamic(line, rec.get("Moneda", default_moneda))
            data.append(rec)
        i += 1
    return data, i

def parse_all_investment_sections(lines: list) -> list:
    sections = [
        {
        ...
        for sec in sections:
            if re.search(sec["section_title"], line, re.IGNORECASE):
                if sec.get("block"):
                    ...
                    block_data, new_i = parse_section(
                        lines, i+1, sec["section_title"], sec["total_title"],
...
def parse_movements_in_block(lines: list, start_idx: int, nombre: str, rut: str, cuenta: str) -> tuple:
    movs = []
    i = start_idx
    ...
    while i < len(lines):
        ...
        if m:
            ...
            if None in [uds_val, px_val, monto_val]:
                i += 1
                continue
...
def process_file_to_data(file_path: Path, output_txt: Path) -> tuple:
    ...
    for inv in inversiones:
        registro = {
            "Fecha": fecha_obj,
            "Nombre": re.sub(r"\.$", "", re.sub(r"^Cuenta", "", nombre_global).strip()),
            "RUT": rut_global,
            "Cuenta": cuenta_global,
            "Nemotecnico": inv.get("Nemotecnico", ""),
            "Moneda": inv.get("Moneda", "CLP"),
            "ISIN": "",
            "CUSIP": "",
            "Cantidad": parse_number(inv.get("Cantidad", "")),
            "Precio_Mercado": parse_number(inv.get("Precio_Mercado", "")),
            "Valor_Mercado": parse_number(inv.get("Valor_Mercado", "")),
            "Precio_Compra": parse_number(inv.get("Precio_Compra", "")),
            "Valor_Compra": "",
            "interes_Acum": "",
            "Contraparte": "BTG pactual",
            "Clase_Activo": inv.get("Clase_Activo", "")
        }
    ...
    while i < len(lines):
        ...
        if re.match(r"^Cartola de Movimientos", l, re.IGNORECASE):
            parsed, new_i = parse_movements_in_block(lines, i+1, cartera[0]["Nombre"], rut_global, cuenta_global)
            movs.extend(parsed)
            i = new_i
        else:
            i += 1
...
def process_btg_files(input_path: Path, output_path: Path):
    ...
    cartera_cols = ["Fecha", "Nombre", "RUT", "Cuenta", "Nemotecnico", "Moneda", "ISIN", "CUSIP",
                    "Cantidad", "Precio_Mercado", "Valor_Mercado", "Precio_Compra", "Valor_Compra",
                    "interes_Acum", "Contraparte", "Clase_Activo"]
    
    movs_cols = ["Fecha Movimiento", "Fecha Liquidación", "Nombre", "RUT", "Cuenta", "Nemotecnico",
    ...
    out_file = output_path / f"InformeBTG_{file_date}.xlsx"
